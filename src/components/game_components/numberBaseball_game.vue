<template>
<div :style="{'background-image':'url(./gameAssets/numberBaseball/Image/bg2.jpg)', 'background-size': '300px'}"
>
    ...
    <div class="row">
        ...
        <h1>{{result}}</h1>
        <form @submit.prevent="onSubmitForm" text-center> <!-- e.preventdefault가 자동 적용 -->
            <input ref="answer" maxlength="4" v-model="value"/>
            <button>입력</button>
        </form>
        <div>시도 : {{tries.length}}</div> <!-- 배열 길이 -->
        <ul>
            <li v-for="t in tries" v-bind:key="t.try"> <!--배열의 값들을 표현하기 위해 v-for(반복문) 사용 -->
                <div>{{t.try}}</div>
                <div>{{t.result}}</div>
                </li>
        </ul>
...
export default { // 이게 import로 가져오는 곳
    data(){
        ...
        value:'',
        result: '',
        imgurl: "./gameAssets/numberBaseball/Image/bg2.jpg",
        logo: "./gameAssets/numberBaseball/Image/bg5.jpg"
        }
    },
    methods: {
        onSubmitForm(e){
...
<style src="./gameCss/numberBaseball_game.css"></style>
